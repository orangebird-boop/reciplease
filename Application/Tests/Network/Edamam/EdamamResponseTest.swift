...
final class EdamamResponseTest: XCTestCase {
...
"""
    func test_shouldDecodeFromJSON () {
        ...
        do {
            ...
            XCTFail("Failed to decode raw JSON data")
        }
    }
    
    func test_shouldTransformEdamamResponseToGenericModel() {
        ...
        do {
            let sut = try JSONDecoder().decode(EdamamResponse.self, from: rawData)
            let recipeResponse = sut.toGenericModel()
            XCTAssertTrue(true)
        } catch {
